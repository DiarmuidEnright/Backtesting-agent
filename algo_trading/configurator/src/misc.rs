...
/// Gets the value of an EditView by its ID from the Cursive instance
pub fn get_value_by_id(id: &str, s: &mut Cursive) -> Option<String> {
    let mut result = None;
    s.call_on_name(id, |view: &mut EditView| {
        result = Some(view.get_content().to_string());
    });
    result
}
